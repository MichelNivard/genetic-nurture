...
pop <- scale(rep(0,8000),rep(1,2000))

mother_e <- sqrt(2)*pop + sqrt(.8)*rnorm(n)
father_e <- sqrt(.2)*pop + sqrt(.8)*rnorm(n)
child_e <- sqrt(.2)*pop + sqrt(.8)*rnorm(n)
sib_e <-   sqrt(.2)*pop + sqrt(.8)*rnorm(n)
adoptee_e <- sqrt(.2)*pop + sqrt(.8)*rnorm(n)
...
mother_t2  <- sqrt(.5)*mother_g  + sqrt(.5)*mother_e
father_t2  <- sqrt(.5)*father_g  + sqrt(.5)*father_e
child_t2   <- sqrt(.5)*child_g   + sqrt(.1)*mother_t2 + sqrt(.1)*father_t2 + sqrt(.3)*child_e  
sib_t2     <- sqrt(.5)*sib_g     + sqrt(.1)*mother_t2 + sqrt(.1)*father_t2 + sqrt(.3)*sib_e
adoptee_t2 <- sqrt(.5)*adoptee_g + sqrt(.1)*mother_t2 + sqrt(.1)*father_t2 + sqrt(.3)*adoptee_e
